...
float *alloc_1d_dbl_device(n)
int n;
{
  float *new;
  cudaMalloc((void **) &new, sizeof(float) * n);
  if (new == NULL) {
    printf("ALLOC_1D_DBL: Couldn't allocate array of floats\n");
    return (NULL);
  }
...
float **alloc_2d_dbl_device(m, n)
int m, n;
{
  int i;
  float **new;

  cudaMalloc((void **) &new, sizeof(float *) * m);
  // new = (float **) malloc ((unsigned) (m * sizeof (float *)));
  if (new == NULL) {
    printf("ALLOC_2D_DBL: Couldn't allocate array of dbl ptrs\n");
    return (NULL);
